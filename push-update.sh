...
updates_dir=/data/pixelage_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have pixelage-VERSION-DATE-TYPE-* as format"
...
# Assume pixelage-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.pixelage.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.pixelage.updater/databases/updates.db" \
